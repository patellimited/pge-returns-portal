...
export default function AdminDashboard() {
  ...
  const submitActionWithAttachment = async ({ mode, returnId, note, file }) => {
        ...
        <div className="fixed inset-0 z-50 flex" data-testid="detail-drawer">
          ...
          <div className="w-full sm:max-w-xl bg-white h-full overflow-y-auto border-l border-[hsl(var(--border))]">
            ...
            <div className="p-4 sm:p-6 space-y-5">
              ...
              <div>
                <div className="label-caps mb-2">Return address</div>
                <div className="text-sm mono">
                  {selected.return_address?.name}<br />
                  {selected.return_address?.street1}<br />
                  {selected.return_address?.city}, {selected.return_address?.state} {selected.return_address?.zip}
                </div>
              </div>
                ...
                {selected.status === "awaiting_approval" && (
                  <>
                    <button
                      ...
                      onClick={() => setActionModal({ mode: "approve", returnId: selected.id, rma: selected.rma_number, email: selected.email })}
                      data-testid="approve-free-btn"
                    >
                      <CheckCircle size={16} className="mr-2" weight="fill" />
                      {selected.method === "store_credit" ? "Approve & issue store credit" : "Approve & send label"}
                    </button>
                    <button
                      className="btn-secondary border-[hsl(var(--destructive))] text-[hsl(var(--destructive))]"
                      onClick={() => setActionModal({ mode: "reject", returnId: selected.id, rma: selected.rma_number, email: selected.email })}
...
function ActionModal({ mode, rma, email, onClose, onSubmit }) {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" data-testid={`action-modal-${mode}`}>
      ...
      <div className="relative bg-white border border-[hsl(var(--border))] w-full max-w-lg shadow-xl">
        <div className="flex items-start justify-between p-5 border-b border-[hsl(var(--border))]">
          <div>
            <div className="label-caps" style={{ color: isApprove ? "hsl(var(--success))" : "hsl(var(--destructive))" }}>
              {isApprove ? "Approve free return" : "Reject return"}
            </div>
        ...
        <div className="p-5 space-y-5">
          <div>
            <label className="label-caps block mb-2">
              {isApprove ? "Note to customer (optional)" : "Reason — shown to customer"}
            </label>
            <textarea
          ...
          <div className="text-xs text-[hsl(var(--ink-muted))] leading-relaxed bg-[hsl(var(--surface))] p-3">
            This {isApprove ? "note and label" : "reason and any image"} will be emailed to <span className="mono text-[hsl(var(--ink))] break-all">{email}</span> immediately after you confirm.
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-2 p-5 border-t border-[hsl(var(--border))]">
          ...
          >
            {submitting ? "Sending…" : isApprove ? "Approve & send label" : "Reject & notify"}
          </button>
        </div>
      </div>
    </div>
