...
import { ArrowUpRight, ArrowRight, Package, MapPin, Lock, Clock, QrCode, CheckCircle, Sparkle } from "@phosphor-icons/react";
...
export default function Landing() {
  ...
  // Real "happy returns" count from the DB. We hide the badge entirely on a
  // fresh install (count == 0) so the hero never shows "0 happy returns".
  const [happyReturns, setHappyReturns] = useState(null);
  useEffect(() => {
    let alive = true;
    api.get("/stats/public")
      .then((r) => { if (alive) setHappyReturns(Number(r.data?.happy_returns || 0)); })
      .catch(() => { if (alive) setHappyReturns(0); });
    return () => { alive = false; };
  }, []);
  const happyDisplay = (() => {
    if (happyReturns == null || happyReturns <= 0) return null;
    if (happyReturns >= 1000) return `${(happyReturns / 1000).toFixed(happyReturns >= 10000 ? 0 : 1)}k+`;
    if (happyReturns >= 100) return `${Math.floor(happyReturns / 10) * 10}+`;
    return String(happyReturns);
  })();
          ...
          <div className="grid grid-cols-12 gap-6 lg:gap-10">
            ...
            <div className="col-span-12 lg:col-span-5 relative">
              ...
              >
                ...
                />
                {/* Dynamic stat badge — "X happy returns processed".
                    Hidden entirely on a fresh install (count == 0). Renders
                    a subtle pill with a sparkle so it feels meaningful, not
                    like a cheap "Live" placeholder. */}
                {happyDisplay && (
                  <div
                    className="absolute top-5 left-5 flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] sm:text-[11px] uppercase tracking-[0.18em] sm:tracking-[0.22em]"
                    style={{
                      background: "hsla(40, 20%, 96%, 0.94)",
                      color: "hsl(30 6% 10%)",
                      backdropFilter: "blur(8px)",
                      boxShadow: "0 6px 18px -10px rgba(27,26,25,0.35)",
                    }}
                    data-testid="happy-returns-badge"
                  >
                    <Sparkle
                      size={12}
                      weight="fill"
                      style={{ color: "hsl(38 75% 45%)" }}
                      aria-hidden
                    />
                    <span className="mono font-medium" data-testid="happy-returns-count">
                      {happyDisplay}
                    </span>
                    <span className="hidden sm:inline">happy returns processed</span>
                    <span className="sm:hidden">happy returns</span>
                  </div>
                )}
              </div>
