...
import { ArrowLeft, CheckCircle, Truck, Package } from "@phosphor-icons/react";
          ...
          <div className="mt-12">
            {data.method === "self_ship" && (
              <div className="mb-8">
                <SelfShipPanel
                  doc={data}
                  needsTracking={data.status === "awaiting_tracking" && !data.self_ship_submitted_at}
                  pendingApproval={data.status === "awaiting_approval"}
                  shipped={!!data.self_ship_submitted_at}
                  onSubmitted={(updated) => setData((prev) => ({ ...prev, ...updated }))}
                />
              </div>
            )}
