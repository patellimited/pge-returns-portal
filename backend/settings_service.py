...
SETTINGS_KEYS = [
    ...
    # Public URL of the return portal (used in customer emails so they can
    # click straight back to their return — e.g. https://returns.pgelimited.com).
    "portal_public_url",
    # Return policy
    ...
    "admin_route_prefix",        # custom URL segment for the admin panel (no leading slash)
    "login_max_attempts",        # failed-login attempts allowed per window
    "login_window_minutes",      # window length in minutes
    "login_lockout_minutes",     # lockout duration in minutes once exceeded
]
...
async def update_settings(db, updates: Dict[str, Any]) -> Dict[str, str]:
    ...
    for k, v in updates.items():
        if k in SETTINGS_KEYS and v is not None and v != "":
            val = str(v).strip()
            # Sanitize the admin route prefix: strip slashes, allow only
            # url-safe chars (a-z, 0-9, dash, underscore). If the result
            # is empty we fall back to "admin" so the panel never becomes
            # inaccessible due to a typo.
            if k == "admin_route_prefix":
                import re
                slug = re.sub(r"[^a-zA-Z0-9_-]", "", val.strip("/")).lower()
                val = slug or "admin"
            # Coerce numeric security knobs into safe ranges.
            if k in ("login_max_attempts", "login_window_minutes", "login_lockout_minutes"):
                try:
                    n = int(float(val))
                except (TypeError, ValueError):
                    n = 5 if k == "login_max_attempts" else 15
                if k == "login_max_attempts":
                    n = max(1, min(n, 100))
                else:
                    n = max(1, min(n, 1440))  # cap window/lockout at 24h
                val = str(n)
            clean[k] = val
    if clean:
